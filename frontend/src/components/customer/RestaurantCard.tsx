...
export function RestaurantCard({ restaurant, getTimeAgo, onShare, language = 'en', transactions = [], onTransactionClick }: RestaurantCardProps) {
      ...
      <CardContent className="p-4">
        ...
        <div className="bg-primary/5 rounded-lg p-3 mb-3">
          <div className="flex items-center justify-between mb-2">
            <div>
              ...
              <p className="text-lg font-bold text-foreground leading-tight">{formatCurrency(balance)}</p>
            </div>
            ...
            {totalUnrealizedEarnings > 0 && (
              <div className="bg-amber-100 dark:bg-amber-900/30 border border-amber-300 dark:border-amber-700 rounded-md px-2 py-1">
                <p className="text-[9px] text-amber-700 dark:text-amber-400 font-medium leading-tight">💰 {t.restaurantCard.earnUpTo}</p>
                <p className="text-sm font-bold text-amber-900 dark:text-amber-300 leading-tight">{formatCurrency(totalUnrealizedEarnings)}</p>
              </div>
            )}
          </div>
        </div>
        ...
        {transactions.length > 0 && (
          <div className="mt-3">
            ...
            >
              <span>{t.restaurantCard.recentActivity} ({transactions.length})</span>
                        ...
                        <div className="text-right flex items-center gap-2">
                          <div>
                            ...
                            {transaction.is_first_transaction && (
                              <p className="text-[10px] text-green-600 dark:text-green-400">{t.recentTransactions.firstVisit}</p>
                            )}
                          </div>
                          {/* Tap indicator */}

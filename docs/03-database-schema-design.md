...
**Purpose:** Each user generates unique referral codes per restaurant to track which restaurant they're actively promoting. This prevents code reuse across restaurants and ensures accurate promotion tracking.
...
**Notes:**
- Each user can generate ONE code per restaurant
...
- When a user wants to promote a new restaurant, they generate a new unique code
- `is_active` allows for code regeneration if user wants a new code for the same restaurant

**Example:**
```
User Alice generates codes for different restaurants:
- Restaurant A (Mamak's Corner): CHILLI-MAMAKS-ABC123
- Restaurant B (Nasi Lemak Place): CHILLI-NASIL-XYZ789
- Restaurant C (Satay Station): CHILLI-SATAY-DEF456
...
**Example Flow:**
```
1. Alice generates code for Restaurant X: CHILLI-RESTOX-ABC123

...
const allowedOrigins = ["https://www.makantak.com", "https://makantak.com", "http://localhost:5173"];
...
Deno.serve(async (req) => {
  // Build CORS headers from the incoming request origin
  ...
  try {
    ...
    if (!restaurantId) {
      return new Response(JSON.stringify({ success: false, error: "Restaurant ID is required" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const { data: rateLimitAllowed, error: rateLimitError } = await supabaseAdmin.rpc("check_ai_chat_rate_limit", {
      p_user_id: user.id,
    });
    ...
    if (!rateLimitAllowed) {
      ...
      const { data: rateLimitStatus } = await supabaseAdmin.rpc("get_ai_chat_rate_limit_status", {
        p_user_id: user.id,
      });
    ...
    if (currentSessionId) {
      ...
      // Create new session
      if (!summary) {
        return new Response(JSON.stringify({ success: false, error: "Dashboard summary required for new session" }), {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          await supabaseUser.from("ai_chat_messages").insert({
            session_id: currentSessionId,
            role: "model",
